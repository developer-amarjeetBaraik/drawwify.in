...
const ArrowSide = () => {
    ...
    const {currArrowHead, setCurrArrowHead} = useContext(toolbarComponentsValueContext)

    const handleBtnClick =(event)=>{
        let value = event.currentTarget.value
        if(!currArrowHead.includes(value)){
            let newArray = [...currArrowHead]
            newArray.push(value)
            setCurrArrowHead(newArray)
        }else{
            let newArray = currArrowHead.filter((items)=> items !== value)
            setCurrArrowHead(newArray)
        }
    }
    ...
    return (
        <div className={style.arrowBtnDiv}>
            <button onClick={handleBtnClick} value={'atStart'} className={`${style.arrowBtn} ${currArrowHead.includes('atStart') ? style.activeBtn:null}`}>
            ...
            <button onClick={handleBtnClick} value={'atEnd'} className={`${style.arrowBtn} ${currArrowHead.includes('atEnd') ? style.activeBtn:null}`}>
