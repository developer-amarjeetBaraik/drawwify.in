...
const ArrowSide = () => {
    ...
    const {currArrowHeadDir, setCurrArrowHead} = useContext(toolbarComponentsValueContext)

    const handleBtnClick =(event)=>{
        let value = event.currentTarget.value
        if(!currArrowHeadDir.includes(value)){
            let newArray = [...currArrowHeadDir]
            newArray.push(value)
            setCurrArrowHead(newArray)
        }else{
            let newArray = currArrowHeadDir.filter((items)=> items !== value)
            setCurrArrowHead(newArray)
        }
    }
    ...
    return (
        <div className={style.arrowBtnDiv}>
            <button onClick={handleBtnClick} value={0} className={`${style.arrowBtn} ${currArrowHeadDir.includes(0) ? style.activeBtn:null}`}>
            ...
            <button onClick={handleBtnClick} value={1} className={`${style.arrowBtn} ${currArrowHeadDir.includes(0) ? style.activeBtn:null}`}>
