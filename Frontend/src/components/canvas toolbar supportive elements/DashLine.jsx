...
const DashLine = () => {
  ...
  const { currDashLine, setCurrDashLine } = useContext(toolbarComponentsValueContext)

  return (
    <div className={`${style.dashLineIconDiv}`}>
      <input checked={currDashLine ? true : false} type="checkbox" name="dashLineBtn" id="dashLineBtn" readOnly />
      <label onClick={() => setCurrDashLine(!currDashLine)} htmlFor="dashLineBtn" className={`${style.dashLineIcon}`}>
