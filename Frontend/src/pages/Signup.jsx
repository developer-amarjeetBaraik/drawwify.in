...
const Signup = () => {
    ...
    const { authenticated, loginViaGoogle, signupViaEmail } = useContext(userAuthContext)
    ...
    const handleLoginBtnClick = (event) => {
        ...
        console.log('clicked')
        signupViaEmail(emailInputRef.current.value, passwordInputRef.current.value, (res, badRes, error) => {
            if(res){
        ...
        <section className='min-h-screen text-white overflow-hidden relative bg-background flex flex-col '>
            ...
            <div className='min-w-[350px] min-h-[400px] h-3/5 mx-auto my-auto bg-glass rounded-lg border-glassBorder flex flex-col items-center'>
                ...
                <div className='mb-7 text-center'>
                    ...
                    <h3 className='text-xl'>Signup to {getComputedStyle(root).getPropertyValue('--brand-name')}</h3>
                </div>
                {/* social buttons */}
                <div className='w-[60%] '>
                ...
                <form className='w-[60%] flex flex-col justify-center items-center' onSubmit={handleLoginBtnClick}>
                    ...
                    <button type='submit' className='mt-3.5 px-2.5 py-1.5 bg-accentLight rounded-lg text-center cursor-pointer'>Signup</button>
                </form>

                <p className='mt-5 text-[12px]'>Already sign-up? <NavLink to={'/auth/login'} className={'text-blue-400 cursor-pointer'}>Go to login</NavLink></p>
