...
const CanvasDrowStoreCopy = ({ children }) => {
  ...
  // Section-Elements drawing helper functions

  // clear the canvas
  const clearCanvas = (whichCanvas)=>{
    if(whichCanvas === 'topCanvas'){
      let ctx = topCanvasRef.current.getContext('2D')
      ctx.clearRect(0, 0, topCanvasRef.current.width, topCanvasRef.current.height)
    }else if(whichCanvas === 'middleCanvas'){
      let ctx = middleCanvasRef.current.getContext('2D')
      ctx.clearRect(0, 0, middleCanvasRef.current.width, middleCanvasRef.current.height)
    }else if(whichCanvas === 'bottomCanvasRef'){
      let ctx = bottomCanvasRef.current.getContext('2D')
      ctx.clearRect(0, 0, bottomCanvasRef.current.width, bottomCanvasRef.current.height)
    }
  }

  // draw rectangle
  const drawRectangle = (color, strokeColor, x, y, width, height, rectRadius = 10,) => {
    const ctx = topCanvasRef.current.getContext('2D')

    ctx.fillStyle = color;
    ctx.strokeStyle = strokeColor;
    ctx.beginPath();
    ctx.roundRect(x, y, width, height, rectRadius)
    ctx.stroke()
    if (color !== null) {
  ...
  // draw circle
  const drawCircle = (color, strokeColor = 'gray', x, y, radius) => {
    const ctx = topCanvasRef.current.getContext('2D')
    ...
    drawExistingElementsOnDrawingCanvasRef.current = () => {
      ...
      elements.forEach(item => {
        if (item.type === 'rectangle') {
          ...
          let strokeStyle = item.strokeColor;
          let x = item.x
          let y = item.y
          let width = item.width
          let height = item.height

          drawRectangle(color, strokeStyle, x, y, width, height)
        }
          ...
          ctx.font = item.font;
          ctx.textBaseline = 'middle'
          ctx.fillText('middle', item.text, item.x, item.y)
        }
        ...
        selectedElements.forEach(item => {
          ...
          if (item.type === 'rectangle') {
            ...
            lines.forEach((line, index) => {
              ctx.fillText(line, x, y + index * (lineHeight || 20));
            });
          } else if (item.type === 'pencil') {
            ctx.beginPath();
            const [start, ...points] = controlPoints || [];
            if (start) {
  ...
  return (
    <>
      <drawCanvasContext.Provider value={{ elements, selectedElements, topCanvasRef, middleCanvasRef, bottomCanvasRef, drawExistingElementsOnDrawingCanvas, drawSelectionArea, drawSelectedElementIndicator, drawSelectedElementsWhenIndicatorsReRendering, drawNewItemOnCanvas, addItemOnCanvas, addSelectedElementsInElementsArray }}>
        {children}
...
export default CanvasDrowStoreCopy
